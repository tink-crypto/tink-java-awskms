...
cat <<'END_DO_AS_TINKUSER' > /tink_orig_dir/tink_java_awskms/_do_as_tinkuser.sh
#!/bin/bash
set -euo pipefail
...
# Compare the dependencies of the ":tink-awskms" target with the declared
...
echo "Building tink-awskms-snapshot-bundle"
echo "========================================"
bazelisk build :tink-awskms-snapshot-bundle
cp bazel-bin/tink-awskms-snapshot-bundle.zip exported_bundles
...
if [[ -f "gpg_pin.txt" ]]; then
  ...
  bazelisk build :tink-awskms-release-bundle
  cp bazel-bin/tink-awskms-release-bundle.zip exported_bundles
fi

END_DO_AS_TINKUSER
### ==================================================== END: _do_as_tinkuser.sh

chmod +x /tink_orig_dir/tink_java_awskms/_do_as_tinkuser.sh

echo "=== COPYING /tink_orig_dir to /home/tinkuser"
cp -r /tink_orig_dir/tink_java_awskms /home/tinkuser
chown --recursive tinkuser:tinkgroup /home/tinkuser/tink_java_awskms
cd /home/tinkuser/tink_java_awskms
